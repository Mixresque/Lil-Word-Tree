 ...
 <body>
  ...
  <?php
    ...
    if (mysqli_connect_errno()) {
      ...
      if ($mysqli->multi_query("CALL CommonEngWords('$topic','$topk');")) {
        if ($result = $mysqli->store_result()) {

            printf("<h2>Show top-k most common words for a certain topic: </h2>\n");
            echo "<table border=1 cellpadding=10>\n";
            echo "<tr> <th>word</th> <th>total frequency</th> </tr>\n";
            while($row = $result->fetch_array()) {
              printf("<tr> <td>%s</td> <td>%s</td> </tr>\n", $row["lName"], $row["totalFreq"]);
            }
            echo "</table>\n";
