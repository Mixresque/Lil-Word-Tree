 ...
 <body>
  ...
  <?php
    ...
    if (mysqli_connect_errno()) {
      ...
      if ($mysqli->multi_query("CALL ShowDerivationAndSynonyms();")) {
        if ($result = $mysqli->store_result()) {

            printf("<h2>Find  word  pairs  such  that  one  word  is  derived  from  the  other,  and  they  are  alsosynonyms.: </h2>\n");
            echo "<table border=1 cellpadding=10>\n";
            echo "<tr> <th>derivation</th> <th>base</th> </tr>\n";
            while($row = $result->fetch_array()) {
              printf("<tr> <td>%s</td> <td>%s</td> </tr>\n", $row["derivation"], $row["base"]);
            }
            echo "</table>\n";
